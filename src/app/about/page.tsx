...
export default function AboutPage() {
  ...
  return (
    <main>
      ...
      <section className="content-section content-section-compact section-dark"><div className="shell"><h2 className="content-heading" style={{ marginBottom: 48 }}>Why one person beats an agency.</h2><div className="about-points">{points.map((point, index) => <div className="about-point" key={point}><span>0{index + 1}</span>{point}</div>)}</div></div></section>
      <section className="content-section content-section-compact section-deep"><div className="shell difference-grid"><h2 className="content-heading">Remote, clear, and honest.</h2><div><p className="content-lead">Everything happens over email, video and screen-share, so it makes no difference whether you're in London, Dublin or Limassol. Scope is agreed before anything starts. I'll tell you what your business actually needs — including when that's less than you were about to pay for.</p><ButtonLink href="/contact">Start a project</ButtonLink></div></div></section>
    </main>
  );
}

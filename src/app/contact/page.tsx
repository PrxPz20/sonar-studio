...
import type { ContactIntent, ContactTier } from "@/lib/contact-schema";
...
const tierBySlug: Record<string, ContactTier> = {
  essential: "Essential",
  standard: "Standard",
  premium: "Premium",
};

const intentBySlug: Record<string, ContactIntent> = {
  project: "Project enquiry",
  teardown: "Free teardown",
};

export default async function ContactPage({ searchParams }: { searchParams: Promise<{ tier?: string | string[]; intent?: string | string[] }> }) {
  const instagram = process.env.NEXT_PUBLIC_INSTAGRAM_URL;
  const linkedin = process.env.NEXT_PUBLIC_LINKEDIN_URL;
  const { tier, intent } = await searchParams;
  const selectedTier = typeof tier === "string" ? tierBySlug[tier.toLowerCase()] : undefined;
  const selectedIntent = typeof intent === "string" ? intentBySlug[intent.toLowerCase()] : undefined;
  return (
    <main>
